...
def apply_edits(original, edits, suggested_level):
    ...
    for edit in edits:
        ...
        try:
            old = candidate.get(name, b'').decode('utf-8')
        except UnicodeError:
        ...
        candidate[name] = new.encode('utf-8')
        changed.add(name)
    if len(changed) >= 100 or len(changed) > 20:
        raise ReviewRequired('想定を超えるファイル数の変更です。')
    if len(changed) > 1:
        level = 2
    actual = {n for n in candidate if candidate[n] != original.get(n)}
    if actual != changed:
        raise ReviewRequired('変更が相殺されたか、差分が一致しません。')
    return candidate, sorted(changed), level
